...
find.para <- multiroot(power.eq, 
                       start = -0.1, 
                       para = c(0.483, -0.022, 696), 
                       positive = FALSE)
?multiroot
# Given a vector of n variables, and a set of n (nonlinear) equations 
# in these variables, estimates the root of the equations, 
# i.e. the variable values where all function values = 0. 
...
b1 <- log(0.5)
n <- 696 
p_0 <- exp(b0a) / (1 + exp(b0a))
# p_y0 = exp(b00)/(1+exp(b00));
p_1 <- exp(b0a + b1) / (1 + exp(b0a + b1))
lambda_s <- n * (p_x * (1 - p_x) * (p_1 - p_0)^2) / 
  (p_x * p_0 * (1 - p_0) + 
     (1 - p_x) * p_1 * (1 - p_1))
lambda_s
...
power.eql <- function(b1, para) {
  ...
  lambda_0 <- n * (p_x * (1 - p_x) * (p_y1 - p_y0)^2) /
    (p_x * p_y0 * (1 - p_y0) + 
       (1 - p_x) * p_y1 * (1 - p_y1)) - 7.85
}

find.para.80 <- multiroot(power.eql,
                          start = -1,
                          para = c(-0.173, -0.022, 696, 0.483), 
                          positive = FALSE)
...
find.para.90 <- multiroot(power.eql,
                          start = -1,
                          para = c(-0.173, -0.022, 696, 0.483), 
                          positive = FALSE)
...
b1 <- log(0.75)
n <- 696
p0 <- exp(b0a) / (1 + exp(b0a))
p1 <- exp(b0a + b1) / (1 + exp(b0a + b1))
p_star <- (1 - p_x) * p0 + p_x * p1
b_tilde <- log(p_star / (1 - p_star))
b_tilde 
## for b_tilde you can use b_0|H_0(b00 = -0.173) 
## directly estimated from null model of HPTN data
D_star <- 2 * (p_x * (p1 * (b0a + b1) - 
                        p_star * b_tilde -
...
fx <- function(x, lambda) {
  dchisq(x, 1, ncp = lambda, log = FALSE)
}

power.eql <- function(lambda, power = 0.8) {
  integrate(fx, 0, 3.84, lambda = lambda)$value - (1 - power)
}

## (1) lambda for 80% power
power_80 <- rootSolve::multiroot(power.eql,
                                 start = 10, 
                                 positive = TRUE, 
                                 power = 0.8)$root
# [1] 7.846775
## (2) lambda for 90% power
power_90 <- rootSolve::multiroot(power.eql, 
                                 start = 10, 
                                 positive = TRUE, 
                                 power = 0.9)$root
# [1] 10.5049
...
plot(x, dchisq(x, 1, 
  ...
  main = expression("Probability density function of" ~ 
                      chi[1]^2 ~ 
                      "with non centrality parameter" ~ 
                      lambda))
lines(x, 
      dchisq(x, 1, ncp = round(power_80, 2), log = FALSE), 
      type = "l", 
      col = 2, 
      lwd = 2)
lines(x, 
      dchisq(x, 1, ncp = round(power_90, 2), log = FALSE), 
      type = "l", 
      col = 3, 
      lwd = 2)
## bquote(lambda == .(round(power_80, 2)))
legend_labels <- c(expression("Central"),
                   ## inside bquote() evaluates the R expression 
                   ## and inserts its value
                   bquote("Non-central with" ~ lambda == .(round(power_80, 2))),
                   bquote("Non-central with" ~ lambda == .(round(power_90, 2))))
legend("topright", 
